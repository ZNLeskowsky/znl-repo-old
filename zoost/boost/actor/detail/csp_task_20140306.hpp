...
//#include <boost/actor/mpsc_queue.hpp>
...
public:
    ~queueable();
protected:
    queueable() : next_(0), status_(0) {}
private:
    queueable* next() { return next_; }
    void set_next(queueable* pobj) { next_ = pobj; }
...
{
protected:
    mpsc_queue() : head_(&dummy_), tail_(&dummy_) {}
    //TODO: check whether exchange() needs to be done in a loop
    void push(queueable* pobj) { tail_->exchange(pobj)->set_next(pobj); }
    queueable* try_pop();
private:
    queueable* head_;
    atomic<queueable*> tail_;
    queueable dummy_;
};
...
public:
    ...
    //    queueable* task;
    //    while (0 == (task = mpsc_queue::try_pop())); // busy wait
    //    return static_cast<queueable_task*>(task);
    //}
};
