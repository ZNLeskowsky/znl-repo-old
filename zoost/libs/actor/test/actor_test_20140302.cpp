...
int main( int, char* [] )
{
    ...
    actor::task_scheduler actor_scheduler;
    thread::thread scheduler_thread(boost::ref(actor_scheduler));
    std::cerr << "started thread with actor scheduler " << &actor_scheduler << std::endl << std::flush;
#endif //BOOST_ACTOR_HAS_SCHEDULER

#ifdef BOOST_ACTOR_HAS_SCHEDULER
    actor::task_runner actor_runner(actor_scheduler);
#else
...
#ifdef BOOST_ACTOR_HAS_SCHEDULER
    TestActor actorD(actor_scheduler);
    TestActor actorE(actor_scheduler);
#endif //BOOST_ACTOR_HAS_SCHEDULER

    actor::send(actorA, &TestActor::test0);
    actor::send(actorB, &TestActor::test1, 1);
#ifdef BOOST_ACTOR_HAS_SCHEDULER
    actor::send(actorD, &TestActor::test1, 1);
    actor::send(actorE, &TestActor::test2, 2, 2.5);
#endif //BOOST_ACTOR_HAS_SCHEDULER
...
#ifdef BOOST_ACTOR_HAS_SCHEDULER
    std::cerr << "stopping actor scheduler " << &actor_scheduler << std::endl << std::flush;
    actor_scheduler.queue_stop();
    std::cerr << "joining scheduler thread" << std::endl << std::flush;
    scheduler_thread.join();
    std::cerr << "joined scheduler thread" << std::endl << std::flush;
#endif //BOOST_ACTOR_HAS_SCHEDULER
